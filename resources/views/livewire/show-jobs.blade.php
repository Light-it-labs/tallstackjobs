...
<!--         <span class="block rounded-md" wire:click="handleSearch">
    ...
    <div class="max-w-screen-sm mx-auto min-w-0 md:px-3 mb-5">
        <template x-for="(job, index) in {{ json_encode($jobs->toArray()['data']) }}" :key="index">
            <div class="w-full flex justify-between border-b last:border-b-0 px-2 md:px-0 py-5 cursor-pointer" x-on:click="current = job; showModal = true">
    ...
    <div class="max-w-screen-sm mx-auto md:px-3 min-w-0 mb-5">
        {{ $jobs->links() }}
    </div>
